      ...
      {showResetDashboardConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="reset-dashboard-dialog-title">
          <div className="card max-w-md w-full shadow-xl">
            ...
            <p className="text-muted-standalone text-sm mb-6">Reset all dashboard settings to their default values and save immediately. Miner IP is kept.</p>
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => setShowResetDashboardConfirm(false)} disabled={savingDashboard} className="btn-ghost">
                Cancel
              </button>
              <button type="button" onClick={handleResetDashboard} disabled={savingDashboard} className="btn-primary">
                {savingDashboard ? 'Resetting…' : 'Reset'}
              </button>
            </div>
          </div>
        </div>
