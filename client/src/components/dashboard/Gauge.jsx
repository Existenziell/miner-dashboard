...
export default function Gauge({ label, value, sub, color = 'text-accent', percent = null }) {
  ...
  return (
    <div className="gauge-card hover:border-accent/20 transition-colors flex flex-col items-center">
      ...
      <div className={`relative w-40 h-40 md:w-52 md:h-52 max-w-full flex shrink-0 ${color}`} aria-hidden>
        <svg
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <span className="text-base sm:text-2xl font-bold leading-tight tabular-nums">{value}</span>
          {sub && (
            <span className="text-xs text-muted mt-0.5 max-w-[90%] truncate hidden md:block">
              {sub}
            </span>
          )}
        </div>
      </div>
      <div className="text-muted font-medium -mt-8 text-center">{label}</div>
    </div>
  );
}
